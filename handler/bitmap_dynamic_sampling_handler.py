...
@initialize_level(1)
class BitmapDynamicSamplingHandler(Handler):
    """
    [get]
        predicted_groups
        group_bitmaps
        ...
        samples
    """
    def __init__(self, sampling_size=100, load_dir="bitmaps") -> None:
        ...
        for table_name in dataset.table_names:
            ...
            for group in predicted_groups:
                ...
                if count >= self.sampling_size:
                    continue
                group_bitmap: bitarray = group_bitmaps.get(group)
                if not group_bitmap:
                    continue
                num_of_rare_groups += 1
                rows = list(group_bitmap.search(bitarray('1')))
