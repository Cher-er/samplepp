...
from gen_model.vaeac.prob_utils import normal_parse_params
...
class VAEAC(Module):
    ...
    def generate_samples_params(self, batch, mask, K=1, xt=None, use_cuda=False, T=0):
        """
        ...
        for i in range(K):
            ...
            ########
                # 参考 DeepGen 拒绝采样
                normal = Normal(0, 1)
                p_z = normal.log_prob(latent)
    
                x_distr = Normal(torch.mean(sample_params, axis=0), 1)
                p_x_z = x_distr.log_prob(xt)
    
                q_z_x = prior.log_prob(latent)
